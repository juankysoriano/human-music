...
export function loadMidi(loaded: () => void) {
   window.MIDI = MIDI || {}
   const instruments = ["acoustic_grand_piano", "acoustic_grand_piano"]
   MIDI.loadPlugin({
      soundfontUrl: "https://juankysoriano.github.io/midi-js-soundfonts/MuseScore/",
      targetFormat: "mp3",
      instrument: instruments,
      onsuccess() {
         instruments.forEach((instrument, index) => MIDI.programChange(index, MIDI.GM.byName[instrument].program))
         loaded()
      },
   })
}
...
export const noteOff = (instrument: number, note: number) => MIDI.noteOff(instrument, note, 0.75 + randomNumber(-0.05, 0.05))
